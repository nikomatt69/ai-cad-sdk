import { AIAnalyticsEvent, AIModelType } from '../../types/index';
...
export class AIAnalytics {
  ...
  constructor(options?: { endpoint?: string; enabled?: boolean }) {
  ...
  trackEvent(event: Omit<AIAnalyticsEvent, 'timestamp'>): void {
    ...
    // Send the event to the backend if configured
  ...
  private sendToAnalyticsBackend(event: AIAnalyticsEvent): void {
    ...
    try {
      // Use Beacon API if available for efficient analytics sending
