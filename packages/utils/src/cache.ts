...
export class AICache {
  ...
  getKeyForRequest(payload: any): string {
    try {
      ...
      // 1. Encode the UTF-8 string to a byte array
      let hash = 0;
      ...
      // Handle error by returning an invalid key to avoid caching
  ...
  set<T>(key: string, data: T, customTtl?: number): void {
    ...
    if (this.cache.size >= this.options.maxSize) {
      // Delete the oldest or expired item
    ...
    // Save the cache if persistence is enabled
    if (this.options.persistToDisk) {
      this.saveToStorage();
    }
  }
  ...
  private estimateMemoryUsage(): number {
    ...
    this.cache.forEach((item) => {
      // Approximate estimate of each item's size
...
export interface CacheOptions {
  ...
  storageKey?: string; // key for localStorage (if persistence is enabled)
